...
| **검색 결과** | `SearchResult`, `SupplierResult` | 판정을 마치고 공급사별 상태·건수와 함께 합친 것. **내부 형태다** |
| **검색 응답** | `SearchResponse` | 위를 응답 계약 모양으로 다시 담은 것. 내부 형태를 그대로 내보내지 않아, 내부 타입이 바뀌어도 계약이 따라 바뀌지 않는다 |

| 말 | 코드 이름 | 뜻과 헷갈리기 쉬운 것 |
|---|---|---|
| **정규화** | `CatalogNormalizer`, `AvailabilityNormalizer` | 받은 값에서 쓸 것과 제외할 것을 가르는 일. 목록 쪽과 재고·요금 쪽이 하나씩 있다.<br>값이 쓸 만한지는 값을 담는 객체가 판정한다 ([ADR-0041](adr/0041-validate-in-constructors.md)). 정규화는 만들어 보고 못 만든 것을 모은다 |
...
| **검색 결과 항목** | `AvailableRoomType` | 판정을 마친, 한 공급사의 숙소 하나의 객실 타입 하나. 내부 식별자·이름·최대 수용 인원·예약 가능 객실 수·요금을 가진다 |
