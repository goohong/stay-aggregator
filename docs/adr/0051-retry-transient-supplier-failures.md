...
- **(가) 공급사 실패 예외에 "재시도 가능한가"를 담는다** ← 채택 — 예외 타입이 하나로 남아 ADR-0027 의 "consumer 에게 같은 예외로 보인다"가 깨지지 않는다.
  대신 공급사 실패 예외가 커지고, 그 값을 채우는 자리가 둘로 나뉜다
...
- **무엇이 일시적인가**
  ...
  | **정한 시간 안에 오지 않음** | **아니요** | **"공급사가 느리다"는 신호다. 느린 공급사를 재시도해도 느릴 가능성이 높다** |
...
- **값(시도 횟수·최소 백오프·최대 백오프)은 설정에 두고 지금은 임시값이다.** 근거를 만들어 정하는 것은 선택 항목을 마친 뒤에 한다
