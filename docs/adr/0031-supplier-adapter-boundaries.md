...
어댑터의 책임 범위
- **(가) 호출·수신·표준 모델 변환·판정까지 전부** — 대신 모든 공급사에 같은 판정 규칙([ADR-0023](0023-multi-night-availability-minimum.md), [ADR-0027](0027-spec-violation-handling-criteria.md))을 공급사마다 다시 구현한다.
...
- 어댑터는 호출, DTO 수신, **공급사마다 다른 것**(실패 표현의 차이, 필드 이름, 요금 구조)을 공통 형태로 바꾸는 데까지 한다. 모든 공급사에 같은 판정 규칙(ADR-0023 의 최솟값, ADR-0027 의 날짜 목록 위반)은 어댑터 밖 공통 로직에 둔다
...
같은 판정 규칙을 공급사마다 구현하면 공급사가 늘수록 규칙이 어긋날 곳이 늘어난다.
...
- 판정 규칙이 한 곳에 있어 규칙을 바꿀 때 한 곳만 고친다
...
- ADR-0027 의 첫 질문(응답 전체를 읽을 수 있나)은 실패 표현이 공급사마다 달라 어댑터에서, 두세 번째 질문은 공통 로직에서 답한다. 판정 기준이 두 곳에 나뉜다
...
- **공급사 식별자를 공통 형태에도 실을지.** 지금은 `CatalogAdapter.supplierId` 와 `FetchedCatalog.supplierId` 두 곳에 같은 상수를 적는다.
  실행 중에 갈릴 경로는 없지만(두 값이 한 상수에서 나온다) 새 공급사를 붙일 때 같은 값을 네 자리에 적어야 하고,
  가짜 어댑터는 식별자와 응답을 따로 받아 손으로 맞춘다. 재고·요금 공통 형태를 만드는 검색 단위에서 같은 질문이 다시 나오므로 그때 한 번에 정한다.
  후보는 "공통 형태에서 빼고 서비스가 어댑터 값을 넘긴다"와 "공통 형태는 공급사를 모른다를 규칙으로 박는다"다
