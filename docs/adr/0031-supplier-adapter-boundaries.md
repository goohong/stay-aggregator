...
목록 조회의 반환 타입
- **(가) `Mono`** ← AI 추천 — 한 클래스 안에서 두 기능의 모양이 같고, 타임아웃을 같은 방식으로 걸고, 공급사들을 함께 호출할 수 있다. 대신 기다리는 사람이 없는 백그라운드 작업에 Reactor 개념이 들어간다.
...
**이유** — 두 consumer 는 서로 다른 결정 묶음을 갖고 있고, 검색이 목록 갱신을 호출하지 않는다는 ADR-0014 를 좁은 경계가 타입으로 강제한다.
