...
**아직 정하지 못한 것** — 문법은 맞지만 목록을 담는 필드가 없는 응답을 어떻게 볼지가 정해지지 않았다. 지금은 공급사마다 다르게 처리된다.

- 공급사 A 가 `{}` 를 주면 빈 목록이 된다. 그러면 그 공급사의 숙소가 모두 목록에서 빠진 것으로 표시된다(ADR-0037)
- 공급사 B 가 `{}` 를 주면 결과 코드가 성공이 아니라 실패가 된다. B 가 빈 목록이 되는 것은 `{"resultCode":"0000"}` 처럼 결과 코드는 성공인데 목록만 없을 때다

어댑터의 `orEmpty()` 가 "필드가 없다"를 "빈 목록"으로 바꾸어, 내부 로직이 둘을 구분할 신호가 남지 않는다.
이 ADR 의 Decision 은 틀의 판정도 내부 로직에서 하기로 했으므로 그 자리를 다시 봐야 한다. 정한 뒤 여기에 적는다.
