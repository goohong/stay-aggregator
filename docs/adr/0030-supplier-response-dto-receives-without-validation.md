...
공급사 응답은 우리가 정한 규약이 아니라 외부 API 의 응답이다. 규약과 다른 응답을 어떻게 판정할지는
...
**이유** — 외부 응답은 우리 규약이 아니므로, DTO 에서 잘라 내면 공급사 장애를 일부라도 허용하려 할 때 손쓸 자리가 없다. 판단이 들어간 규칙은 받는 틀인 DTO 의 역할이 아니고,
