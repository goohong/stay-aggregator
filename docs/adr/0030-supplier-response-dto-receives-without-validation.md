---
id: 0030
title: 공급사 응답 DTO 는 스펙 필드를 모두 받기만 하고 검증은 내부 로직에 두기로 결정
status: accepted
date: 2026-09-15
---

# 0030. 공급사 응답 DTO 는 스펙 필드를 모두 받기만 하고 검증은 내부 로직에 두기로 결정

## Context

공급사 응답은 우리가 정한 스펙이 아니라 외부 API 의 응답이다. 스펙과 다른 응답을 어떻게 분류할지는
[ADR-0027](0027-spec-violation-handling-criteria.md) 이 정했지만, 그 검증을 응답 DTO 에서 할지 내부 로직에서 할지는 정하지 않았다.
목록 불일치 감지([ADR-0014](0014-detect-drift-in-search-correct-in-sync.md))는 조회 응답의 이름·최대 수용 인원을 쓴다.
...
필드 범위
- **스펙의 모든 필드를 받는다** — 선택지를 따로 두지 않았다. 목록 불일치 감지처럼 출력에 직접 쓰지 않는 필드도 쓰임이 있고, 받을지 말지를 필드마다 판단하지 않는다.
...
- **(나) 응답 구조(`items`, `resultCode`, `data`)는 DTO 에서 엄격하게, 항목 필드는 받은 뒤 검사한다** — AI 가 처음 권했다. 구조 검증이 역직렬화 설정에 묶인다.
- **(다) DTO 는 검증하지 않고 받기만 하며, 구조까지 포함한 검증을 내부 로직에서 한다** ← 최종 — 검증 규칙이 한곳에 모인다. 대신 DTO 필드가 null 허용 타입이 되어 스펙상 필수 여부가 타입으로 드러나지 않는다.

필드 이름 표기
- **(가) 응답 예시대로 `hotelCode` 만 받는다** ← 최종 — 실제로 `HotelCode` 로 오면 A 의 항목이 모두 제외되고 뺀 사유로 남는다.
...
- DTO 는 검증하지 않는다. 필드는 null 을 허용해 받고, 스펙상 필수인 필드는 주석으로 적는다. 응답 구조를 포함해 빠진 필드·null 의 검증은 DTO 를 받은 뒤 내부 로직에서 ADR-0027 의 세 질문으로 한다
- 역직렬화 단계에서 실패하는 것은 JSON 문법 오류와 타입 불일치뿐이고, 이는 ADR-0027 의 첫 질문에서 공급사 실패로 본다
- 필드 이름은 응답 예시의 표기(`hotelCode`)대로 받는다

**이유** — 외부 응답은 우리 스펙이 아니므로, DTO 에서 잘라 내면 공급사 장애를 일부라도 허용하려 할 때 손쓸 곳이 없다. 판단이 들어간 규칙은 응답을 받는 구조인 DTO 의 역할이 아니고,
...
**얻는 것**
- 공급사 응답의 검증 규칙이 내부 로직 한곳에 모이고, ADR-0027 의 세 질문을 그대로 코드로 옮길 수 있다
...
- 공급사 A 가 실제로 `HotelCode` 로 보내면 A 의 결과가 모두 제외된다. 동기화 로그의 제외 건수로 드러난다

## Discussion

- **AI 주장과 근거** — 모르는 필드는 Jackson 3 기본값이라 설정 없이도 무시된다고 설명했다. 빠진 필드는 응답 구조를 DTO 에서 엄격하게 두자고 권했고, 숙소 코드 표기 불일치에는 별칭으로 두 표기를 모두 받자고 권했다
- **반박**
...
- **검증 결과** — 명시 방법은 DTO 애너테이션과 전역 설정 두 가지였고, 전역 설정은 우리가 받는 요청까지 관대해졌다. DTO 에서 구조를 엄격하게 검사하면 ADR-0027 의 검증 일부가 역직렬화 설정으로 흩어졌다.
  표기 불일치는 표의 나머지 필드와 응답 예시가 모두 소문자로 시작해 표기 실수로 보는 편이 근거가 있었고, 별칭은 근거 없는 대비였다
- **그래서 어떻게 바뀌었나** — 모르는 필드는 DTO 에 명시, 검증은 구조까지 내부 로직으로, 필드 이름은 응답 예시대로 정했다
...
목록을 담는 필드가 없는 응답을 어떻게 볼지는 [ADR-0041](0041-validate-in-constructors.md) 에서 정했다.
이 ADR 이 "구조 검증도 내부 로직에서"라고만 두고 어디서 할지 정하지 않은 탓에 어댑터의 `orEmpty()` 가 검증을 대신하고 있었고, 그 ADR 이 검증 위치까지 함께 정했다.
