...
`app/src/main/kotlin/com/stayaggregator` 아래 패키지가 넷인지. `supplier` 와 `mapping` 안의 파일이 다른 패키지를 import 하지 않는지.
검색을 만든 뒤 `search` 가 `catalog` 를 import 하지 않는지. 확인은 위 `grep` 이다.
