...
- **(나) 컨트롤러가 `Mono` 반환** — 기다리는 동안 Tomcat 스레드를 놓아준다. 대신 공급사 응답 뒤의 처리가
  ...
  필터가 ASYNC 디스패치에서 불리지 않고, 테스트에 `asyncDispatch` 가 필요하며, Tomcat 30초 비동기 타임아웃과
  우리 타임아웃을 함께 맞춰야 한다.
...
- **매핑 읽기가 늦어지면 검색 전체가 그만큼 늦어진다** (2026-09-17 구현하며 확인).
  별도 스케줄러를 두지 않으므로 그 읽기는 요청 스레드에서 블로킹으로 일어나고, 검색에 건 시간 한계가 그 구간은 끊지 못한다.
  끊으려면 스케줄러를 두어야 하는데 그것은 이 결정이 하지 않기로 한 것이다. DB 가 응답하지 않는 상황은 커넥션 풀의 대기 한계가 다룬다
