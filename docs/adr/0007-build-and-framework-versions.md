...
- **(가) 2.1.0** — 처음 골격을 만들 때 쓴 값. 근거는 없었다.
- **(나) 2.4.20** ← AI 추천 — 최신이고, Spring Boot 4.1.1 의 BOM 과 stdlib 버전이
  어긋나지 않는 것을 실측했다. Kotlin 은 2.1→2.4 사이에 이 프로젝트가 쓰는 범위
  (data class, null 안전성, Spring 연동)에 알려진 호환성 문제가 없다.
