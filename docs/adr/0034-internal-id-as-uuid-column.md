...
**잃는 것**
- 식별자 형식을 바꾸면 테이블과 인덱스를 다시 쓰는 마이그레이션이 필요하고 그동안 잠금이 걸린다. 우리 규모에서는 초 단위로 관측됐다(위 측정)
...
- **검증 결과** — 그 관행은 실재하지만 적용 대상이 다르다. 값 범위를 강제하는 CHECK 제약, DB 의 열거 타입, 외래 키처럼 **업무 규칙이 스키마에 박히는 것**에는 잘 맞는다.
