...
- **(ㄴ) 어댑터마다 만든다** — 지금 모양이다. 설정이 늘 때마다 어댑터 수만큼 고친다
...
**얻는 것**
- 연결이 안 되는 공급사는 3.1초 안에 실패한다. 지금 재고 호출 타임아웃(5초)이나 목록 호출 타임아웃(30초)까지 기다리지 않는다
- WebClient 설정을 바꿀 곳이 한 곳이다

**잃는 것**
- 공급사 설정값이 하나 는다
- 3.1초는 우리 공급사를 재서 정한 값이 아니다. 공급사가 멀리 있어 연결에 오래 걸리면 정상 공급사도 실패할 수 있다
