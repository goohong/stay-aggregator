...
WebClient 를 만드는 곳
- **(ㄱ) `supplier` 패키지의 함수 하나로 만든다** ← 채택 — 연결 설정이 한 곳에 있다. (함수를 부르지 않는 어댑터를 막지는 못했다. [ADR-0072](0072-adapter-http-helper-and-archunit.md) 의 ArchUnit 규칙 `WebClient 는 SupplierWebClient 에서만 만든다` 가 그것을 막는다)
...
연결이 수립되지 않으면 호출 타임아웃까지 기다리지 않고 실패하는지
  ...
  (라우트가 없는 환경에서는 연결 타임아웃이 아닌 실패로 끝나 건너뛴다. 그래서 아래 두 테스트가 네트워크 없이 같은 것을 나눠 본다)

연결 타임아웃 설정값이 HTTP 클라이언트에 들어가는지, 연결 타임아웃 실패가 시간 초과로 분류되는지
  → `SupplierAvailabilityAdapterTest.공급사 HTTP 클라이언트에 설정한 연결 타임아웃이 들어간다`
  → `SupplierFailureTest.연결 타임아웃으로 난 요청 실패는 시간 초과로 표시되고 재시도 가능하지 않다`
