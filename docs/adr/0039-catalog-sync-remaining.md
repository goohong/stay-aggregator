...
결정과 상충하는 곳 세 가지가 나왔고, 그중 둘은 여기서 정해야 할 갈림길이었다.
...
- 계획에 타임아웃이 없었다. Mock 의 무응답은 10분간 붙잡고 스케줄러는 기본 스레드 하나라("The ThreadPoolTaskScheduler uses one thread by default",
  ...
  공급사 하나가 멈추면 다른 공급사와 이후 주기까지 멈춘다. [ADR-0019](0019-first-catalog-failure-no-special-handling.md) 와 상충한다
...
공급사별 설정 블록 안에 둔다([ADR-0038](0038-catalog-sync-execution.md) 이 정한 그 설정 블록이다). 코드에 기본값을 두지 않고, 설정에 값이 없으면 앱이 기동에 실패하게 한다.

- **이유** — 공급사마다 응답 속도가 다르면 느린 쪽 때문에 빠른 쪽의 실패 감지가 늦어진다. 어댑터가 이미 자기 설정 블록을 받으므로 타임아웃도 같은 설정 블록에 있다
...
- **이유** — 90초를 걸어도 중간 장비가 대개 60초 안팎에서 먼저 연결을 종료하므로 그 값이 하는 일이 없다. 30초는 우리가 정한 타임아웃이 되고, 단일 스레드에서 다른 공급사를 막는 시간도 줄어든다.
  목록 동기화는 하루 한 번 도는 백그라운드 작업이라 30초 안에 오지 않는 응답을 기다릴 이유가 없다
- **30초라는 숫자 자체는 문서에 없다.** "중간 장비 기본값보다 짧게, 백그라운드 작업이므로 여유 있게"라는 우리 판단이다
...
- **다시 볼 것** — 실제 공급사를 붙이면 관측한 지연 분포로 정한다. 공급사가 목록을 쪽 단위로 나눠 주면 호출 타임아웃과 동기화 전체 타임아웃을 따로 두어야 한다
...
- **"이번 목록에 없는 숙소 수"는 반영이 끝난 뒤에 센다.** 반영은 그 공급사 행을 먼저 모두 표시하고 이번 목록에 있는 것의 `missing_since` 를 비우는 순서라(ADR-0037),
...
- 같은 기준을 숙소로 넓혀 "팔 수 있는 객실 타입이 하나도 없는 숙소"를 뺄지는 [ADR-0041](0041-validate-in-constructors.md) 에서 정했다
...
**잃는 것**
- 타임아웃 값이 설정에 없으면 앱이 기동에 실패한다. 처음 띄우는 사람이 설정을 채워야 한다
- 뺀 응답의 원본은 남지 않는다
- 테스트를 동시에 돌릴 수 없다
...
- **검증 결과** — 지적이 맞다. 인용한 Boot 문서를 원문과 대조했다. 값이 정해지기 전에도 타임아웃 설정 키는 있어야 하고, 추측한 기본값을 두지 않으려면 설정을 필수로 두는 방법이 맞았다
...
타임아웃으로 취소하는지
  → `SupplierCatalogAdapterTest.공급사가 응답하지 않으면 타임아웃으로 취소한다`

타임아웃된 공급사만 실패하고 다른 공급사의 매핑은 반영되는지
