...
검색 한 건이 들어오면 매핑에서 숙소 코드를 꺼내 공급사별로 모아 재고·요금 API 를 부른다.
...
숙소 코드가 50개를 넘으면 chunk 로 나눠 호출하는지
  → `SearchServiceIntegrationTest.숙소가 50개를 넘으면 chunk 로 나눠 호출한다`
...
동시 실행 수가 실제로 걸리는지, 재고 호출 타임아웃이 끊는지
  → [검색 지연 측정](../research/search-latency.md). 지연 2초에 chunk 4개는 2.05초, 8개는 4.04초. 지연 7초는 5.01초에 끊긴다
