...
- **Mock 이 숙소 코드 수를 검사해 50개를 넘으면 공급사처럼 오류를 준다.** A 는 HTTP 400 과 그 오류 이름으로, B 는 HTTP 200 과 본문 코드로 각자 형식대로 거절한다
...
- Mock 코드가 는다. 검증 수단을 늘리는 것이라 "Mock 자체는 평가 대상이 아니다"라는 요구사항과 상충하지 않지만,
  어디까지 모사할지 선은 필요하다. **공급사 스펙에 오류로 적힌 것까지만 모사하고 그 밖은 하지 않는다**

**다시 볼 것**
- 동시 실행 수와 검색 전체 타임아웃의 값. 관측한 지연 분포로 정한다. 그때 풀 크기도 함께 본다 (지금 값은 ADR-0068)
- **호출 한도(429·`E429`)를 어떻게 다룰지.** 한도 수치가 스펙에 없어 지금은 정하지 않는다.
  동시 실행 수를 제한하는 것이 1차 방어이고, 한도에 걸렸을 때의 처리는 재시도·서킷과 함께 검토한다 (재시도 횟수와 대기는 ADR-0068 에서 정했다)
...
- **반박** — Mock 에 검사를 더하는 것이 "Mock 에 시간을 쓰지 말라"와 부딪히지 않느냐는 AI 의 우려에 대해,
  그 제약은 Mock 의 완성도를 보지 않으니 거기에 시간을 들이지 말라는 뜻이고, 우리 구현을 확인하는 가장 확실한 방법이면 쓰는 것이 맞다고 정리했다.
  요구사항도 Mock 을 연동 동작 검증 수단으로는 본다고 적고 있다
...
동시 실행 수가 실제로 걸리는지, 재고 호출 타임아웃이 호출을 취소하는지
  → [검색 지연 측정](../research/search-latency.md). 지연 2초에 chunk 4개는 2.05초, 8개는 4.04초. 지연 7초는 5.01초에 타임아웃으로 취소된다

동시 실행 수·검색 전체 타임아웃·재고 호출 타임아웃의 **값**은 측정으로 정하지 못했다. 측정이 보여 준 것은 세 값이 숙소 수와 묶여 있다는 관계식이고,
숙소 수와 실제 공급사의 지연 분포를 모르고는 정할 수 없다. 값은 관계식과 인용·판단으로 [ADR-0068](0068-search-values-from-relations.md) 에서 정했고 그 확인도 거기에 있다.
