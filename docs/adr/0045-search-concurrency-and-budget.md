---
id: 0045
title: 재고·요금 호출을 50개씩 나누고 동시 실행 수와 검색 전체 타임아웃을 두기로 결정
status: accepted
date: 2026-09-17
---

# 0045. 재고·요금 호출을 50개씩 나누고 동시 실행 수와 검색 전체 타임아웃을 두기로 결정
...
동시 실행 수와 검색 전체 타임아웃
- **(가) 동시 실행 수와 검색 전체 타임아웃만 정하고 풀 크기는 기본값을 쓴다** ← 채택 — 지금 만질 근거가 있는 것만 정한다. 대신 실제 규모에서 기본 풀이 맞는지 확인하지 못한다
...
- **검색 한 건 전체에 타임아웃을 둔다(검색 전체 타임아웃).** 그 안에 끝내지 못한 공급사는 부분 실패로 내보내고 나머지 결과로 응답한다.
  호출 타임아웃과 별개다. 목록 동기화의 30초([ADR-0039](0039-catalog-sync-remaining.md))는 배경 작업 값이라 검색에 쓰지 않는다
...
**다시 볼 것**
- 동시 실행 수와 검색 전체 타임아웃의 값. 관측한 지연 분포로 정한다. 그때 풀 크기도 함께 본다
...
한 공급사가 검색 전체 타임아웃을 넘겨도 나머지 결과로 응답하는지
  → `SearchServiceIntegrationTest.검색 전체 타임아웃을 넘긴 공급사만 실패가 되고 나머지는 그대로 나간다`
...
동시 실행 수·검색 전체 타임아웃·재고 호출 타임아웃의 **값**은 아직 임시값이다. 측정이 보여 준 것은 세 값이 숙소 수와 묶여 있다는 관계식이고,
