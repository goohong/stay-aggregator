...
- **기록을 다시 시도한다.** 요청함 상태와 멱등 키는 이미 기록돼 있으므로(예약 생성 3) 확정으로 바꾸는 쓰기만 다시 한다
- **다시 시도해도 기록하지 못하면 공급사 예약을 취소한다.** 이것이 보상 트랜잭션이다. 고객에게는 예약이 되지 않았다고 알린다.
...
- **사용자 판단(보상)** — 기록을 다시 시도하고, 그래도 안 되면 공급사 예약을 취소하는 안으로 정했다. 사람에게 넘기는 안은 택하지 않았다
