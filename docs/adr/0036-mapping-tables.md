...
- **(나) 공급사와 숙소 코드를 그대로 들고 있다** — 공급사가 준 세 값으로 한 번에 찾는다. 대신 같은 코드가 두 테이블에 적히고 불일치하지 않게 할 책임이 코드로 넘어온다.
...
쓰기는 목록 동기화 때만 일어나고(기본 하루 한 번, [ADR-0013](0013-catalog-sync-on-startup-and-interval.md)·[ADR-0016](0016-catalog-sync-interval-default-daily.md)) 요청 경로에는 없어서, 무작위 키가 인덱스에 흩어지는 비용이 드러날 여지가 적다.
...
- **검증 결과**
  - 객실 타입 식별자는 실제로 미정이었다. ADR-0011 은 숙소 키 형식으로 적힌 문장이었고, 정해지지 않은 것을 정해진 것처럼 보이게 한 잘못이었다
