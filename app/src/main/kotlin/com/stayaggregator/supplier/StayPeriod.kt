 ...
 * 바깥(우리 API, 공급사 호출)은 두 날짜를 쓰고 안쪽(정규화)은 날짜 하나하나를 쓰므로, 그 변환을 여기서만 한다.
 *
 * 체크인일이 지난 날짜인지는 보지 않는다. 그 검증은 시간대 기준이 정해져야 한다 (Q28).
 */
data class StayPeriod(val checkIn: LocalDate, val checkOut: LocalDate) {
     ...
     *
     * 3박이면 셋이다. 연박 예약 가능 객실 수 계산(ADR-0023)과 빠진 날짜 검증(ADR-0027)이 이 목록과 응답을 대조한다.
     */
