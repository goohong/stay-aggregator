...
/**
 * 어댑터가 재고·요금 응답에서 꺼낸 값. 아직 판정하지 않은 상태다 (ADR-0030, ADR-0031).
 *
 * [FetchedCatalog] 와 같은 규칙이다. 값이 없을 수 있어 모두 null 을 허용하고, 없는 목록과 빈 목록을 구분해서 넘기며,
 * 어느 공급사가 줬는지는 싣지 않는다 (ADR-0049).
 *
 * **요금은 공급사가 준 모양 그대로 둘 중 하나로 온다.** 날짜별 금액을 주는 공급사와 총액을 주는 공급사가 있고,
 * 날짜별 금액을 총액으로 합치려면 요청한 날짜가 다 왔는지 먼저 봐야 한다. 그 판정은 공급사 공통 규칙이라 어댑터 밖에서 하므로
 * (ADR-0031, ADR-0027 의 요금 표), 어댑터는 더하지 않고 받은 대로 넘긴다. 판정을 마친 요금은 `search` 의 `Rate` 부터 같은 모양이다 (ADR-0042).
 */
...
/** 공급사가 준 요금의 모양. 어느 쪽인지는 공급사가 정하고, 하나의 총액으로 만드는 일은 판정 쪽이 한다 */
