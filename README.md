...
**만드는 것** — 공급사의 숙소 목록을 미리 받아 공급사 코드와 내부 식별자의 매핑을 DB 에 저장해 두고,
고객이 날짜와 인원으로 검색하면 여러 공급사의 재고·요금을 동시에 물어 하나의 형식으로 합쳐 돌려줍니다.
공급사 하나가 실패해도 나머지 결과로 응답합니다.

**다루지 않는 것** — 인증·인가, 결제, 관리자 기능, 프론트엔드, 실제 외부 API 연동, 정렬·페이징.
지역·키워드 검색도 하지 않습니다. 공급사가 지역 정보를 주지 않기 때문입니다. 그래서 검색 조건은 날짜와 인원뿐이고 대상은 보유 숙소 전체입니다.

**공급사 API 는 실제 서버 대신 `mock-supplier` 로 모사했습니다.** 정상·장애·무응답·지연을 재현할 수 있어야 타임아웃과 부분 실패가 실제로 도는 것을 보일 수 있기 때문입니다.

완벽한 구현보다 설계와 판단 과정에 무게를 두었습니다. 무엇을 왜 그렇게 정했는지는 [ADR](docs/adr/) 에, 막힌 지점과 바꾼 판단은 [JOURNAL](JOURNAL.md) 에 있습니다.
어디까지 구현했는지는 [선택 항목의 구현 범위](#선택-항목의-구현-범위) 와 [하지 않은 것](#하지-않은-것) 에 적었습니다.
